...
p_dataframe$FC=FC[order(paste(FC$Timepoint, FC$`P53 Status`),
         decreasing =T),3]
levels(p_dataframe$`P53 Status`)=c('p53-Wildtype', 'p53-Mutant')
df2plot$`P53 Status`=factor(df2plot$`P53 Status`)
levels(df2plot$`P53 Status`)=c('p53-Mutant', 'p53-Wildtype')
...
ggplot(df2plot, aes(x=Timepoint,y=mean, fill=sgRNA)) + 
  ...
  geom_text(data=p_dataframe, aes(x=Timepoint,y=y_value,
